...
HOST_USER="${1:-}"
# Default to /root/pos because Snap Docker often cannot read /opt paths.
HOST_DIR="${2:-/root/pos}"
...
ssh "${HOST_USER}@${HOST_IP}" "docker load -i '${HOST_DIR}/pos.tar'"

echo "Running container on remote host..."
